...
public class TestSPH : MonoBehaviour
{
    ...
    private float GAS_STIFFNESS = 15f;//18.0f;
    private float VISCOSITY_COEFFICIENT = 7f;//15f; //3.5f;

    private float SURFACE_TENSION_COEFFICIENT = 0.1456f;//0.0728f;

    private int NUM_OF_PARTICLES = 216;
    ...
    void Start()
    {
        ...
        //--------------------------------------------------------------------------------------------------------------
        ps = new MyParticle[NUM_OF_PARTICLES];
        fParticleMass = MATERIAL_DENSITY * VOLUME / NUM_OF_PARTICLES;
        ...
        */
        ...
        int a = (int)Mathf.Pow(NUM_OF_PARTICLES, 1.0f / 3.0f);
        float l = step * a;
        ...
        //Initialising kernel
        //--------------------------------------------------------------------------------------------------------------
        fSupportRadius = Mathf.Pow(((3 * VOLUME * KERNEL_PARTICLE) / (4 * Mathf.PI * NUM_OF_PARTICLES)), 1.0f / 3.0f);
        ...
        //--------------------------------------------------------------------------------------------------------------
        ht = new SpatialHashTable(NUM_OF_PARTICLES, fSupportRadius);
        ...
        Container = new MyBoxCollider(new Vector3(10f, 10f, 10f), new Vector3(0.6f, 0.6f, 0.6f));
#endif
        //--------------------------------------------------------------------------------------------------------------
    ...
    private void FixedUpdate()
    {
        ...
        {
            Container.UpdatePosition(new Vector3(0.01f, 0,0));
        }
        ...
        {
            Container.UpdatePosition(new Vector3(0, 0.01f, 0));
        }
        ...
        if (bActionLock == false)
        {
            ...
            foreach (MyParticle p in ps)
            {
                ...
                {
                    p.fMassDensity += (n.fMass * Poly6(p.v3Position - n.v3Position));
                ...
                //if (p.id == 0)
                //     Debug.Log("ID: " + p.id + " Pressure: " + p.fPressure+ " Density: " + (p.fMassDensity - REST_DENSITY) + " Counted: " + neighbours.Count);
            }
            ...
            foreach (MyParticle p in ps)
            {
                ...
                foreach (MyParticle n in neighboursF)
                {
                    float pipjmj = (p.fPressure + n.fPressure) * n.fMass;
                    float dj2 = 2 * n.fMassDensity;

                    Vector3 distance = p.v3Position - n.v3Position;
